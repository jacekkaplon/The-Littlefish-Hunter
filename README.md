...
* 👉 [INC-2026-001: EICAR Pipeline Validation(PDF)](./reports/incident/INC-2026-001_Incident_Report.pdf) — End-to-end test of the automated malware triage pipeline: EICAR write triggers SHA-256 integrity alert, VirusTotal API lookup, and automated containment — confirming the detection chain before live exposure.
* 👉 [INC-2026-002: SSH Brute-Force & Post-Compromise Reconnaissance (PDF)](./reports/incident/INC-2026-002_Incident_Report.pdf) — Live brute-force intrusion from Brazil: root access gained, immediate post-exploitation fingerprinting via uname -a within 6 seconds of login. Full kill chain documented from first failed attempt to recon command.
* 👉 [INC-2026-003: Cryptominer Deployment via SSH  (PDF)](./reports/incident/INC-2026-003_Incident_Report.pdf) — Attacker uploaded a 23MB ELF64 binary disguised as sshd and executed it via nohup. Sample confirmed malicious by 37/65 VirusTotal engines. Static analysis: stripped, dynamically linked, consistent with XMRig-family cryptominer.
* 👉 [INC-2026-004: Multi-Source Reconnaissance & Detection Engineering  (PDF)](./reports/incident/INC-2026-004_Incident_Report.pdf) — Four IPs across 13.5 hours running an identical sandbox-evasion fingerprinting script (OS, CPU, GPU profiling + honeypot detection via shell error formatting). Campaign correlated by shared script marker. Custom Wazuh rule defect identified, root-caused, and fixed during the same investigation.
* 👉 [INC-2026-005: Mirai-Style IoT Botnet via Telnet  (PDF)](./reports/incident/INC-2026-005_Incident_Report.pdf) — Mirai-style IoT botnet via Telnet: process killing, filesystem scoring for persistence, multi-arch payload probing (MIPS → ARMv6l). C2 confirmed malicious (VT: 13/91), payload URL disguised as image/jpeg to bypass content filters.
<br>
<br>
